...
int main()
{
 ...
 
    {
        ...
        vector<vector<pair<ll, ll>>> par(n, vector<pair<ll, ll>>(m, {-2, -2}));
        par[a.first][a.second] = {-1, -1};
        vec2 vis(n, vec(m, 0));
 
        bfs(v, a, b, vis, par);
        if (par[b.first][b.second] == make_pair(-2LL, -2LL))
        {
            cout << "NO"<<endl;
        }
        else
        {
            string ans;
            pair<ll, ll> now = {b.first, b.second};
            while (par[now.first][now.second] != make_pair(-1LL, -1LL))
            {
                pair<ll, ll> store = par[now.first][now.second];
                if (store.first < now.first)
                    ans.push_back('D');
                else if (store.first > now.first)
                    ans.push_back('U');
                else if (store.second < now.second)
                    ans.push_back('R');
                else
                    ans.push_back('L');
                now = store;
